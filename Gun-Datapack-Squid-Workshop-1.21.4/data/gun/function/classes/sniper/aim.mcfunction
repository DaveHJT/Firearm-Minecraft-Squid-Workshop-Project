...
execute if score @s aim matches 2 run item modify entity @s weapon.mainhand [{"function":"set_custom_model_data","strings":{"values": ["awm_aim"],"mode":"replace_all"}}, {"function":"minecraft:set_attributes","modifiers":[{"attribute":"minecraft:movement_speed","id":"minecraft:","amount":-0.1,"operation":"add_value","slot":"mainhand"}]}]
