...
function gun:classes/main/util/hitbox_bare
...
#light arrow effects
execute as @s[tag=hit,distance=..1] at @s if entity @e[tag=cbp,tag=light,limit=1,sort=nearest] run function gun:classes/main/util/light_hit
...
#bullet lose damage
execute if entity @s[tag=hit,distance=..1,tag=!special] run scoreboard players remove @e[tag=cbp] damage 2
