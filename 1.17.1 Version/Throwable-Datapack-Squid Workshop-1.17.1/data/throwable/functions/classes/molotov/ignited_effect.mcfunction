...
# particle
particle minecraft:flame ~ ~ ~ 2.5 0.5 2.5 0.0001 6 force

particle minecraft:lava ~ ~ ~ 2.5 0.1 2.5 0.0001 3 force

# sound
execute if score fire throwable_life matches 0 run playsound minecraft:block.fire.ambient ambient @a ~ ~ ~ 3 2

# deal damage
execute if score fire throwable_life matches 0 as @e[distance=..6,type=!#damage:notmob] positioned ~-8 ~-1 ~-8 if entity @s[dy=3,dx=16,dz=16] run function throwable:classes/molotov/damage
